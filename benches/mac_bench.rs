use criterion::{black_box, criterion_group, criterion_main, Criterion};
use crypto::mac::*;

fn bench_poly1305(c: &mut Criterion) {
    let key = [
        0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e,
        0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d,
        0x1e, 0x1f,
    ];
    let message = black_box([128u8; Poly1305::BLOCK_LEN]);

    let mut poly1305 = Poly1305::new(&key);

    c.bench_function("bench_poly1305", |b| {
        //b.bytes = Poly1305::BLOCK_LEN as u64;
        b.iter(|| poly1305.update(&message))
    });
}

fn bench_polyval(c: &mut Criterion) {
    let key = [
        0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e,
        0x0f,
    ];
    let message = black_box([128u8; Polyval::BLOCK_LEN]);

    let mut polyval = Polyval::new(&key);

    c.bench_function("bench_polyval", |b| {
        //b.bytes = Polyval::BLOCK_LEN as u64;
        b.iter(|| polyval.update(&message))
    });
}

fn bench_ghash(c: &mut Criterion) {
    let key = [
        0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e,
        0x0f,
    ];
    let message = black_box([128u8; GHash::BLOCK_LEN]);

    let mut ghash = GHash::new(&key);

    c.bench_function("bench_ghash", |b| {
        //b.bytes = GHash::BLOCK_LEN as u64;
        b.iter(|| ghash.update(&message))
    });
}

criterion_group!(benches, bench_poly1305, bench_polyval, bench_ghash);
criterion_main!(benches);
