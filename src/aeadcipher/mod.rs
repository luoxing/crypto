...
pub trait AeadCipher: Sized {
    ...
    fn aead_encrypt_slice_detached(
        &self,
        nonce: &[u8],
        aad: &[u8],
    ...
    #[must_use]
    fn aead_decrypt_slice_detached(
        &self,
        nonce: &[u8],
        aad: &[u8],
    ...
    ($name:tt, $kind:tt) => {
        impl AeadCipher for $name {
            ...
            fn aead_encrypt_slice_detached(
                ...
                match (nonce.is_empty(), aad.is_empty()) {
                    (true, true) => self.encrypt_slice_detached(
                        &[aad, nonce],
                        plaintext_in_ciphertext_out,
                        tag_out,
                    ),
                    (true, false) => {
                        self.encrypt_slice_detached(&[nonce], plaintext_in_ciphertext_out, tag_out)
                    }
                    (false, true) => {
                        self.encrypt_slice_detached(&[aad], plaintext_in_ciphertext_out, tag_out)
                    }
                    (false, false) => {
                        self.encrypt_slice_detached(&[], plaintext_in_ciphertext_out, tag_out)
                    }
                }
            }
            #[must_use]
            fn aead_decrypt_slice_detached(
                ...
                match (nonce.is_empty(), aad.is_empty()) {
                    (true, true) => self.decrypt_slice_detached(
                        &[aad, nonce],
                        ciphertext_in_plaintext_out,
                        tag_in,
                    ),
                    (true, false) => {
                        self.decrypt_slice_detached(&[nonce], ciphertext_in_plaintext_out, tag_in)
                    }
                    (false, true) => {
                        self.decrypt_slice_detached(&[aad], ciphertext_in_plaintext_out, tag_in)
                    }
                    (false, false) => {
                        self.decrypt_slice_detached(&[], ciphertext_in_plaintext_out, tag_in)
                    }
                }
            }
        }
