...
fn transform(
    ...
    v[0] ^= v[8];
    v[8] ^= chaining_value[0];
    v[1] ^= v[9];
    v[9] ^= chaining_value[1];
    v[2] ^= v[10];
    v[10] ^= chaining_value[2];
    v[3] ^= v[11];
    v[11] ^= chaining_value[3];
    v[4] ^= v[12];
    v[12] ^= chaining_value[4];
    v[5] ^= v[13];
    v[13] ^= chaining_value[5];
    v[6] ^= v[14];
    v[14] ^= chaining_value[6];
    v[7] ^= v[15];
    v[15] ^= chaining_value[7];
...
fn bench_blake3_transform(b: &mut test::Bencher) {
    ...
    b.iter(|| transform(&key, &block, block_len, counter, flags))
}
            ...
            if self.offset == Self::BLOCK_LEN {
                ...
                if self.chunk_len == LEN {
                    ...
                    let flags = if self.chunk_block_counter == 0 {
                        self.flags | CHUNK_START | CHUNK_END
                    } else {
                        self.flags | CHUNK_END
                    };
